      ...
      <!-- Start content -->
             ...
             <div class="container-fluid">
                 ...
                 <div class="row">
					...
					<h6 class="mt-0">Noon & Night and On Duty
...
$.ajax({
    ...
    // return false;
       ...
       if(data.status ==200){
          ...
          $('#loadingDiv').hide();
           ...
           var myarray = ['FTCO','FTCI','HL','HPL','WFH','CORR'];





            if($.inArray(data.attendace.status, myarray) != -1) {
                ...
                var y = TodayDate.getFullYear();
                 ...
                 var predateformet = y+'-'+pm+'-'+day;

                 var currentformat = y+'-'+m+'-'+futurday;

                var preDate =  new Date(predateformet);

                var currentDate =  new Date(currentformat);

                var dateRange = getDates(preDate,currentDate);

                

                 if(jQuery.inArray(checkdate, dateRange) != -1) {
           ...
           $('#error_msg').html('Correction Apply '+ dateRange[0] + ' To ' +dateRange.slice(-1)[0] ).attr('style','color:red');
          ...
          $('#error_msg').html('Correction Not Apply' ).attr('style','color:red');
  ...
  function sendnsrequest(){
    ...
    if(ns_to_data ==''){
      ...
      var d1 = Date.parse(ns_form_data);

                  var d2 = Date.parse(ns_to_data);

                  if (d1 > d2) {

                       $('#error_ns_to_data').text('Enter Riight Date').attr('style','color:red');
