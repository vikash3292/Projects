...
function get_edu_data(Request $request){
  $edu = DB::table('grc_empeducationdetails')->where('id',$request->edu_id)->first();

   return response()->json(['status' => 200, 'edu' => $edu ]);
}
...
public function addappyleave(Request $request){
   ...
   $leavetime = json_decode($request->leavetime);
    ...
    $showuserid = array($userid,$request->manager??0,$request->hr??0,$request->admin??0,1,$request->Director??0);
...
public function leavelist(){
  ...
  $leavelist = DB::table('grc_employeeleaves')->join('main_employeeleavetypes','main_employeeleavetypes.leavecode','=','grc_employeeleaves.leave_mode')->leftJoin('main_users', 'main_users.id', '=', 'grc_employeeleaves.user_id')->leftJoin('total_leave_list', 'total_leave_list.user_id', '=', 'grc_employeeleaves.user_id')->whereRaw("find_in_set($userid,show_user_id)")->orderBy('id','DESC')



  ->select('grc_employeeleaves.*','total_leave_list.total_year_leave','total_leave_list.month_leave','total_leave_list.used_leave','total_leave_list.unpaid_leave','main_users.userfullname','main_employeeleavetypes.leavetype as leave_mode')
...
public function aproveleave(Request $request){
 ...
 $leavecount = $request->leavecount;
  ...
  if($updatedate){
      ...
      if(isset($leaveuser->emailaddress)){
...
if(($leavereject->manager ==1 && $leavereject->hr ==1 && $leavereject->admin==1 || $leavereject->director==1)){
   ...
   private function attedanceStatus($leaveid,$user_id){
    ...
    $leave_details = DB::table('grc_employeeleaves')->where('id',$leaveid)->first();
             ...
             if($leave_details->lwp > 0){
                 ...
                 if($days == 1 && $leave_details->avail >= 0){

                        $staus = "L" ;   

                 }else if($days == 1 && $leave_details->avail >= 1.0){

                      $staus = "L"   ;

                 }else if($days == 2 && $leave_details->avail >= 0.5){
             ...
             }else if($days == 1 && $leave_details->avail == 0){
 ...
 //  dd($request->all());
  ...
  $holi = DB::table('main_holidaydates')->whereDate('org_open_date',$request->selectedDate)->count();
  $fx_holiday = DB::table('main_holidaydates')->where('groupid',2)->whereDate('holidaydate',$request->selectedDate)->count();
    ...
    if($holi > 0){
      ...
      return response()->json(['status' => 202, 'msg' => 'Holi day']);
    ...
    if($dayname == 'Sun' && $holi == 0){



      return response()->json(['status' => 202, 'msg' => 'weekend Off']);



    }elseif ($first_sat == $request->selectedDate) {
    ...
    $checkleave  = DB::table('grc_employeeleaves')->whereDate('leaveDate',$request->selectedDate)->where('user_id',$userid)->count();
    ...
    $myleave =  DB::table('grc_employeeleaves')->where('leave_mode','MyL')->where(DB::raw("(DATE_FORMAT(grc_employeeleaves.leaveDate,'%Y'))"), ">=", date('Y'))->where('user_id',$userid)->count();
  ...
  if($myleave  == 0){
     ...
     return response()->json(['status' => 200, 'msg' => 'added']);
      ...
      return response()->json(['status' => 202, 'msg' => 'already Apply MyL']);

  }

     }

     $past = date("Y-m-d", strtotime("-1 month", strtotime( date('Y-m-d') )));

     $future = date("Y-m-d", strtotime("+1 month",strtotime( date('Y-m-d') )));



     if(strtotime($past) > strtotime($request->selectedDate) &&  strtotime($request->selectedDate) < strtotime($future) ){

           return response()->json(['status' => 202, 'msg' => ' Apply More 1 Month pass and 1 month Future']);
    ...
    // dd($holidatleave->first()->groupid);
      ...
      }else if($holidatleave->count() == 0  && $leavecode == 'PL'){ 
     ...
     // return response()->json(['status' => 203, 'msg' => 'already add Leave']);
      ...
      $userdetails = DB::table('grc_employeeleaves')->join('main_employeeleavetypes','main_employeeleavetypes.leavecode','=','grc_employeeleaves.leave_mode')->leftJoin('main_users','grc_employeeleaves.user_id','=','main_users.id')->leftJoin('main_users as rmuser','main_users.reporting_manager','=','rmuser.id')->where('user_id', $userid)->whereDate('leaveDate',$request->selectedDate)->select('grc_employeeleaves.*','main_users.userfullname','rmuser.userfullname as reportingmanager','main_employeeleavetypes.leavetype')->first();
...
//   }
  ...
  if(isset($user)){
    ...
    foreach($user as  $users){
      ...
      DB::table('main_users')->where('user_id',$users->user_id)->update(['leave_pl' => $users->leave_pl + 1.0,'leave_cl' => $users->leave_cl + 0.5]);
...
function cron_hourly_peding_leave_remainder(){
    ...
    $content = '';
                    ...
                    foreach ($takeleave as $key => $takeleaves) {
                      $apply_user = array();
                      $apply_user[] = $takeleaves->user_id;
            $approved_user = DB::table('leave_approval')->where('leave_id',$takeleaves->id)->get();
              foreach ($approved_user as $key => $value) {
               $apply_user[] = $value->user_id;
              }
                         

                      $approval_user = explode(',', $takeleaves->show_user_id);
                     
                       $approval_user = array_diff($approval_user,$apply_user);



                     $user_name = DB::table('main_users')->where('id',$takeleaves->user_id)->first();

                       $aproval_email = DB::table('main_users')->whereIn('id',$approval_user)->get();
                       $emails = [];
                       foreach ($aproval_email as $key => $aproval_emails) {
                         $emails[] = $aproval_emails->emailaddress;
                       }


                    $content .='Dear '.$user_name->userfullname.' Your Penading by ';

                    sendMail($emails,$subject,$from,$fromname,$content);
...
public function leavetype(Request $request){
    ...
    if(isset($takeleave) && !empty($takeleave)){
        ...
        $takenleave = 0;
         ...
         $total_leave =  $userdetails->leave_pl - $takenleave;
    ...
    if(isset($takeleave) && !empty($takeleave)){
      ...
      $total_leave  =  $userdetails->leave_cl - $takenleave;
...
function get_trainig_data(Request $request){
   ...
   $training = DB::table('main_training')->where('id',$request->tranig_id)->first();


    return response()->json(['status' => 201, 'msg' => 'Something Went to Worng','training' => $training ]);
