...
public function dashboardview(){
    ...
    $announment = DB::table('announcement_master')->where('status',1)->orderBy('id','DESC')->limit(5)->get();
...
$getleavecount = DB::table('grc_employeeleaves');
if($Role !=1){
$getleavecount->where('user_id',$userid);
}

$getleavecount = $getleavecount->where('manager',1)->where('hr',1)->where('admin',1)->where(DB::raw("(DATE_FORMAT(grc_employeeleaves.leaveDate,'%Y'))"), "=", date('Y'))->where('leave_mode','CL')->select(DB::raw('SUM(grc_employeeleaves.leave_type) AS leaveCount'))->first();
...
$dashboardview['CL'] = Auth::guard('main_users')->user()->leave_cl;
$dashboardview['PL'] = Auth::guard('main_users')->user()->leave_pl;
...
$tasknotfound[] = $hideprojectlists->project_id;
 ...
 if($Role != 1){
    ...
    return view('admin.dashboard',['announment' => $announment,'holyDay' => $upcomingholydate,'birthday' => $birthday,'takeleave' =>$takenLeave,'pending' =>$pending,'halfday' =>$halfday,'late' =>$late,'dashboad' => $dashboardview,'approvalList' => $getattence]);
  ...
  // dd($request->all());
    ...
    if($leavecode == 'PL'){
        ...
        for($i = 0;$i < (int)$lesscountmonthless; $i++){
         ...
         $preleave = DB::table('grc_employeeleaves')->where('status',1)
         ...
         ->whereBetween(DB::raw("(DATE_FORMAT(grc_employeeleaves.leaveDate,'%Y-%m'))"), [$startDate, $endDate])->where('leave_mode',(int)$leavecode)->where('user_id',$userid)->orderBy(DB::raw("(DATE_FORMAT(grc_employeeleaves.leaveDate,'%m'))"), 'asc')->groupBy(DB::raw("(DATE_FORMAT(grc_employeeleaves.leaveDate,'%m'))"))->select(DB::raw('SUM(grc_employeeleaves.leave_type) as leaveCount'),DB::raw("(DATE_FORMAT(grc_employeeleaves.leaveDate,'%m'))"))->get();
    ...
    ->where(DB::raw("(DATE_FORMAT(grc_employeeleaves.leaveDate,'%Y-%m'))"), "=", $now)->where('leave_mode',$leavecode)->where('user_id',$userid)->select('leave_type as appliedleavescount')->get();
    ...
    foreach($takeleave as $takeleaves){
     ...
     $Totalavaliableleave =   ($preMonthleaveAvail < 0)?0: $preMonthleaveAvail - $takenleave;
      ...
      if($Totalavaliableleave < 0){
       ...
       return $Totalavaliableleave + 1.5;
...
function shownotification(Request $request){
    ...
    $getnotification = DB::table('main_notification')->leftjoin('main_users as sender','main_notification.sender_id','=','sender.id')->leftjoin('main_users as reciver','main_notification.reciver_id','=','reciver.id')->orderBy('main_notification.create_at', 'DESC')->select('main_notification.*','sender.userfullname as sender_user_name','reciver.userfullname as reciver_username','sender.profileimg','sender.prefix');
     if($role !=1){
       ...
       $leave_data = DB::table('grc_employeeleaves')->leftjoin('main_users as sender','grc_employeeleaves.user_id','=','sender.id')->join('main_employeeleavetypes','main_employeeleavetypes.leavecode','=','grc_employeeleaves.leave_mode')->whereRaw("find_in_set($current_userid,grc_employeeleaves.show_user_id)")->where(function($query)
                {
                ...
                })->where('grc_employeeleaves.status',1)->orderBy('grc_employeeleaves.id','DESC')->select('grc_employeeleaves.*','main_employeeleavetypes.leavetype','sender.userfullname as sender_user_name','sender.profileimg','sender.prefix')->get();
    ...
    foreach($leave_data as $getnotifications){
        
       $cunt = date('d-M-Y', strtotime($getnotifications->leaveDate));

       $leave_url = URL::to('/leavelist');
                 ...
                 $html .=  ' <a href="javascript:void(0);" class="dropdown-item notify-item">
                              ...
                              $html .= '</div>
                              <p class="notify-details">'.ucwords($getnotifications->sender_user_name).'<span class="text-muted"> Leave Apply - '.$cunt.'<a href="'.$leave_url.'">Click here</a></span></p>
                           </a>';                             
    ...
    foreach($allnotidata as $getnotifications){
       ...
       $dashboard = URL::to('/dashboard');
                 ...
                 $html .=  ' <a href="javascript:void(0);" class="dropdown-item notify-item">
                              ...
                              $html .= '</div>
                              <p class="notify-details">'.ucwords($getnotifications->sender_user_name).'<span class="text-muted">'. $getnotifications->msg .' - '.$cunt.'<a href="'.$dashboard.'">Click here</a></span></p>
                           </a>';                             
