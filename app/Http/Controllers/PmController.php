...

{
	...
	public function edit_project_task(Request $request,$id){
	   ...
	   $milestone = DB::table('tm_milestone')->where('project_id',$id)->get();
	   ...
	   $main_category = DB::table('main_project_category')->where('status',1)->get();
        ...
        $newusertasklist= DB::table('main_users')->leftjoin('main_jobtitles','main_users.jobtitle_id','=','main_jobtitles.id')->leftjoin('main_roles','main_users.emprole','=','main_roles.id')->whereIn('main_users.id',$assin)->where('emprole','!=', 1)->where('main_users.isactive',1)->select('main_users.*','main_users.userfullname','main_roles.rolename','main_jobtitles.jobtitlename')->get();
	      ...
	      return view('projects.edit_project',['stage' => $stage,'managerlist' => $managerlist,'currency' => $currency ,'org' => $org,'main_category' => $main_category,'baseproject' => $baseproject,'milestone' => $milestone,'projectedit'=>$projectedit,'taskedit' =>$tastedit,'userlist' => $project_user_list,'newuserlist' =>$newuserlist,'project_id' =>$id,'form_show' => $form_show,'newusertasklist' => $newusertasklist ]);
...
function add_comnication(Request $request){
	...
	DB::table('comnication_log')->insert($arr);
